...
class Register(APIView):
    ...
    class InputSerializer(serializers.Serializer):
        ...
        password = PasswordField(required=True)
        roles = serializers.ListField(child=serializers.CharField(required=True, allow_blank=False, allow_null=False), required=False, allow_null=False)
    ...
    def post(self, request):
        ...
        user = User.objects.filter(Q(primary_email=email) | Q(username=self.serializer_class.validated_data.get('username'))).first()
        if user and user.is_active == False:
            user.is_active = True
            user.username = self.serializer_class.validated_data.get('username')
            user.primary_email = self.serializer_class.validated_data.get('primary_email')
            user.contact_number = self.serializer_class.validated_data.get('contact_number')
        ...
        if user.id:
            ...
            return Response({"message": "User account not created."}, status=status.HTTP_400_BAD_REQUEST)
