...
class CreateCostCenter(APIView):
    ...
    def post(self, request):
        ...
        if self.serializer_class.is_valid():
            cost_center = create_cost_center(**self.serializer_class.validated_data)
            return Response({"message": "New Cost-Center-ID Created Successfully"}, status=status.HTTP_201_CREATED)
