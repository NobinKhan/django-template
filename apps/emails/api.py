...
class SendEmail(APIView):
    ...
    class InputSerializer(serializers.Serializer):
        ids = serializers.ListField(
            child=serializers.PrimaryKeyRelatedField(queryset=Product.objects.all(), allow_null=True),
            allow_empty=False
        )
    ...
    def post(self, request):
        ...
        if self.serializer_class.is_valid():
            ...
            if emails is None:
                return Response({"message": "Please set some emails!"}, status=status.HTTP_404_NOT_FOUND)
            ...
            # first file
            file1 = open(f"{request.user.username}-{timezone.now().microsecond}.txt", 'w', newline='')

            # second file
            file2 = open(f"{request.user.username}-{timezone.now().microsecond}-RFID.txt", 'w', newline='')
            flag = None

            for product in self.serializer_class.validated_data.get('ids'):
                file1.write('   {}          {}   {}{}'.format(f"{product.asset.number}{product.sub_number}{product.created_at.strftime('%Y%m%d')}", request.user.username, sg_time.strftime('%H%M%S'),'\n'))
                if product.is_rfid:
                    flag = True
                    file2.write('   {}          {}   {}   {}{}'.format(f"{product.asset.number}{product.sub_number}{product.created_at.strftime('%Y%m%d')}", request.user.username, sg_time.strftime('%H%M%S'), product.inventory_number,'\n'))

            file1.close()
            file2.close()

            subject, from_email = f'Scanned Data From Asset Verification System (user: {request.user.username})', settings.EMAIL_HOST_USER
            ...
            for product in self.serializer_class.validated_data.get('ids'):
                ...
                product.save()
            return Response({"message": "Email sending!"}, status=status.HTTP_201_CREATED)
