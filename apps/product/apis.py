...
import pandas as pd
from apps.common.detect import validate_barcode, validate_csv_xl
...
class CreateSubNumber(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            create_sub_digit(**serializer.validated_data)
            return Response(
                {"message": "Sub Number Created Successfully"},
...
class CreateUploadProduct(APIView):
    ...
    class InputSerializer(serializers.Serializer):
        ...
        asset = serializers.IntegerField(
            max_value=99999999999999, min_value=100000000000, required=True
        )
        sub_number = serializers.IntegerField(
            max_value=9999, min_value=0, required=True
        )
        ...
        if "multipart/form-data" in content_type:
            ...
            if file_serializer.is_valid():
                serializer = self.InputSerializer(
                    data=file_serializer.list_data,
                    many=True,
                    context={"request": request},
                )
                try:
                    if serializer.is_valid():
                        UploadProduct.objects.all().delete()
                        serializer.save()
                        return Response(
                ...
                if isinstance(errors, list):
                    return Response(
                        [d for d in errors if d], status=status.HTTP_403_FORBIDDEN
                    )
            ...
            if isinstance(request.data, list):
                ...
                if serializer.is_valid():
                    try:
                        ...
                        return Response(
                            {"message": msg.args}, status=status.HTTP_403_FORBIDDEN
                        )
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        return Response(
            {"message": "Invalid Json Format"}, status=status.HTTP_403_FORBIDDEN
        )
...
class ProductList(APIView):
    ...
    def get(self, request):
        ...
        if not client or client not in ["app", "web"]:
            return Response(
                {"message": "Invalid request"}, status=status.HTTP_403_FORBIDDEN
            )
...
class ScanProduct(APIView):
        ...
        def validate_inventory_number(self, value):
            ...
            if not value:
                raise serializers.ValidationError(
                    _("Empty or Null data may not acceptable")
                )
            ...
            if tag is None:
                ...
                instance = Product.objects.filter(
                    asset__number=asset, sub_digit__number=sub_number
                )
            ...
            if instance:
                if instance[0].email_sent_status in [
                    Product.SendStatus.SEND,
                    Product.SendStatus.SEND_USER_DELETED,
                ]:
                    self.invalid_data = {
                        "asset": "already_sent",
            ...
            if tag == "BARCODE":
                asset, sub_number = value.split("-")
                instance = UploadProduct.objects.filter(
                    asset__number=asset, sub_digit__number=sub_number
                )
    ...
    def post(self, request):
        ...
        if isinstance(request.data, list):
            ...
            unique = [
                {"inventory_number": x.replace("*", "").replace("\x00", "")}
                for x in {item for sublist in data for item in sublist}
            ...
            for every in unique:
                try:
                    in_serializer = self.InputSerializer(
                        data=every, context={"request": request}
                    )
            ...
            if response:
                ...
                try:
                    return Response(
                        in_serializer.errors, status=status.HTTP_400_BAD_REQUEST
                    )
                    ...
                    return Response(
                        {
                            "asset": "Not_PUB_asset",
                            "sub_number": "Not_PUB_asset",
                            "inventory_number": in_serializer.initial_data.get(
                                "inventory_number"
                            ),
                            "cost_center": "Not_PUB_asset",
                            "name": "Not_PUB_asset",
                ...
                return Response(
                    {"error": "empty data may not be allowed"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        return Response(
            {"message": "Invalid body data!"}, status=status.HTTP_406_NOT_ACCEPTABLE
        )
...
class GetScannedProduct(APIView):
    ...
    def get(self, request):
        prducts = Product.objects.filter(
            send_by=request.user, email_sent_status=Product.SendStatus.NOT_SEND
        ).order_by("asset__number")
...
class DashBoardDeatilsApi(APIView):
    ...
    def get(self, request):
        ...
        emailsdata = (
            Email.objects.all()
            .exclude(Q(email__icontains="@care-box.com") | Q(role="carebox"))
            .count()
        )
        ...
        for users in user:
            User.objects.filter(username=users["username"]).exclude(
                is_superuser=True
            )
