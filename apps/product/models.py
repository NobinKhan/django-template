...
class SubNumber(BaseModel):
    number = models.PositiveSmallIntegerField(
        verbose_name=_("Sub Number"), unique=True, null=True, blank=True
    )
...
class Tag(BaseModel):
    ...
    tag = models.CharField(
        verbose_name=_("Scanned Tag (Barcode/RFID) "),
        max_length=100,
        unique=True,
        null=True,
        blank=True,
    ...
    tag_type = models.CharField(
        default=None, max_length=10, choices=TagType.choices, blank=True, null=True
    )
...
class Department(BaseModel):
    name = models.CharField(
        verbose_name=_("Department"), max_length=20, null=True, blank=True
    )
...
class UploadProduct(BaseModel):
    ...
    asset = models.ForeignKey(
        Asset, on_delete=models.PROTECT, related_name="products", blank=True, null=True
    )
    sub_digit = models.ForeignKey(
        SubNumber,
        on_delete=models.PROTECT,
        related_name="products",
        verbose_name=_("Sub-Number"),
        blank=True,
        null=True,
    )
    tag = models.ForeignKey(
        Tag, on_delete=models.PROTECT, related_name="products", blank=True, null=True
    )
    ...
    name = models.CharField(
        verbose_name=_("Name/Description"), max_length=200, blank=True, null=True
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["asset", "sub_digit"], name="unique_upload_product"
            )
        ]

    @property
    def sub_number(self):
        if self.sub_digit is None:
            return None
...
class Product(BaseModel):
    ...
    email_sent_status = models.CharField(
        default=SendStatus.NOT_SEND,
        max_length=20,
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["asset", "sub_digit"], name="unique_product"
            )
        ]
