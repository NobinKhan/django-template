...
from apps.common.permissions import ChangeUser, ViewUser, DeleteUser, ViewRole
...
class UpdateUser(APIView):
    ...
    class InputSerializer(serializers.Serializer):
        ...
        contact_number = PhoneNumberField(required=False, allow_blank=False, allow_null=False)
        password = PasswordField(required=False, allow_blank=False, allow_null=False)
        roles = serializers.ListField(child=serializers.CharField(allow_blank=False, allow_null=False), required=False, allow_null=False)
    ...
    def post(self, request, pk):
        ...
        updated_user, has_updated = user_update(user=user, data=serializer.validated_data)

        if has_updated:
            return Response({"message": "User account changed successfully!"})
        else:
            return Response({"message": "User account not update."}, status=status.HTTP_400_BAD_REQUEST)
...
class UserListApi(APIView):
    ...
    class OutputSerializer(serializers.ModelSerializer):
        ...
        class Meta:
            model = User
            fields = ("id", "username", "contact_number", "email", "created_at", "roles")
